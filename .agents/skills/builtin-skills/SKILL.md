...
name: builtin-skills
description:
  Use when creating a new builtin skill for Trapeze, editing an existing builtin
...
Trapeze embeds skills directly into the binary via `internal/skills/builtin/`.
...
- `DiscoverBuiltin()` walks the embedded FS, parses each `SKILL.md`, and sets
  paths with the `trapeze://skills/` prefix (e.g., `trapeze://skills/jq/SKILL.md`).
- The View tool resolves `trapeze://` paths from the embedded FS, not disk.
...
| -------------- | ----------------------- | ------------------------------------------ |
| `trapeze-config` | `builtin/trapeze-config/` | Trapeze configuration help                   |
| `trapeze-hooks`  | `builtin/trapeze-hooks/`  | Authoring, configuring and debugging hooks |
