...
the agent lifecycle, allowing you to both build on top of Trapeze, customize
...
- Trapeze ships with a builtin `trapeze-hook` skill write, edit, and configure
  hooks; just tell Trapeze how to configure Trapeze
- Trapeze currently supports just one hook, `PreToolUse`, with plans to support
...
our **project-level** `trapeze.json`. Relative paths like `./no-haskell.sh` work
here because the project root is your working directory. If you're configuring
a global hook (`~/.config/trapeze/trapeze.json`), use an absolute path instead.
...
# ($TRAPEZE_TOOL_INPUT_COMMAND) to grep and match on a regexp.
if echo "$TRAPEZE_TOOL_INPUT_COMMAND" | grep -qE '(^| )((ghc|cabal|stack)(\.exe)?)( |$)'; then
...
Hooks run through Trapeze's embedded POSIX shell (`mvdan.cc/sh`) — the same
...
- **Permissive shebang fallback**: if the absolute path in a shebang
  doesn't exist (e.g. `#!/bin/bash` on Windows), Trapeze falls back to a
  ...
  "no opinion".
- **Environment**: every hook sees `TRAPEZE=1`, `AGENT=trapeze`, and
  `AI_AGENT=trapeze` on top of the `TRAPEZE_*` hook-specific variables. These
  ...
  both contexts.
- **Timeout behavior**: when a hook exceeds its timeout, Trapeze cancels the
  context and waits a short grace period (~1s) for the interpreter to
  yield. If the hook still hasn't returned, Trapeze abandons it, logs a
...
Hooks can be added to your `trapeze.json` (or `.trapeze.json`) at both the global
...
> work fine in project-level `trapeze.json` because the project root is also
> your working directory. For **global** config (`~/.config/trapeze/`),
> however, you must use either an absolute path or an inline command:
>
> ```jsonc
> // Global ~/.config/trapeze/trapeze.json
> {
>   "hooks": {
>     "PreToolUse": [
>       {
>         "command": "/home/you/.config/trapeze/hooks/no-haskell.sh"
...
3. Runs all matching hooks **in parallel** through Trapeze's embedded POSIX
...
5. Applies the result **before** permission checks. If the aggregated decision
   ...
   for it. If it's `allow`, Trapeze treats that as affirmative pre-approval and
...
however you'll incur some additional overhead as Trapeze will still parse and
run each hook.
...
| ---------------------------- | ---------------------------------------------- |
| `TRAPEZE`                      | Always `1` when running under Trapeze.           |
| `AGENT`                      | Always `trapeze`.                                |
| `AI_AGENT`                   | Always `trapeze`.                                |
| `TRAPEZE_EVENT`                | The hook event name (e.g. `PreToolUse`).       |
| `TRAPEZE_TOOL_NAME`            | The tool being called (e.g. `bash`).           |
| `TRAPEZE_SESSION_ID`           | Current session ID.                            |
| `TRAPEZE_CWD`                  | Working directory.                             |
| `TRAPEZE_PROJECT_DIR`          | Project root directory.                        |
| `TRAPEZE_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command being run. |
| `TRAPEZE_TOOL_INPUT_FILE_PATH` | For file tools: the target file path.          |

The `TRAPEZE`, `AGENT`, and `AI_AGENT` markers are also set by the `bash`
tool, so a script can detect "am I running under Trapeze?" the same way in
either context.
...
Hooks communicate back to Trapeze via **exit code** and `stdout`/`stderr`. The
...
deterministic based on the order hooks appear in `trapeze.json`.
...
If a hook exceeds its timeout, Trapeze cancels its context and treats the
...
if echo "$TRAPEZE_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
returns `decision: "allow"`, which tells Trapeze to pre-approve the call:
...
case "$TRAPEZE_TOOL_INPUT_COMMAND" in
...
if [[ "$TRAPEZE_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
```jsonc
{ "command": "echo \"$(date -Iseconds) $TRAPEZE_TOOL_NAME\" >> ./tools.log" }
```
...
Trapeze hooks are broadly compatible with [Claude Code
...
hooks run under Trapeze unchanged. This document covers the Trapeze-specific API
only — anything not documented here isn't guaranteed to work.

One intentional divergence: Trapeze treats `updated_input` as a shallow-merge
...
4. `decision` precedence: `deny` > `allow` > `null`. First deny determines the
   ...
   is `allow`, Trapeze pre-approves the tool call and skips the permission
