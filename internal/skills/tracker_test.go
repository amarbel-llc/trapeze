...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (trapeze-config).
	activeSkills := []*Skill{
		{Name: "trapeze-config", Description: "Trapeze config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("trapeze-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via trapeze://...).
	tracker.MarkLoaded("trapeze-config")
	require.True(t, tracker.IsLoaded("trapeze-config"))
