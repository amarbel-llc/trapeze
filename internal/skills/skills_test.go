...
func TestToPromptXMLBuiltinType(t *testing.T) {
	...
	skills := []*Skill{
		{Name: "builtin-skill", Description: "A builtin.", SkillFilePath: "trapeze://skills/builtin-skill/SKILL.md", Builtin: true},
		{Name: "user-skill", Description: "A user skill.", SkillFilePath: "/home/user/.config/trapeze/skills/user-skill/SKILL.md"},
	}
	xml := ToPromptXML(skills)
...
func TestDiscoverBuiltin(t *testing.T) {
	...
	for _, s := range discovered {
		if s.Name == "trapeze-config" {
			found = true
			require.True(t, strings.HasPrefix(s.SkillFilePath, BuiltinPrefix))
			require.True(t, strings.HasPrefix(s.Path, BuiltinPrefix))
			require.Equal(t, "trapeze://skills/trapeze-config/SKILL.md", s.SkillFilePath)
			require.Equal(t, "trapeze://skills/trapeze-config", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	}
	require.True(t, found, "trapeze-config builtin skill not found")

	var foundJQ bool
	for _, s := range discovered {
		if s.Name == "jq" {
			foundJQ = true
			require.Equal(t, "trapeze://skills/jq/SKILL.md", s.SkillFilePath)
			require.Equal(t, "trapeze://skills/jq", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	...
	var foundHooks bool
	for _, s := range discovered {
		if s.Name == "trapeze-hooks" {
			foundHooks = true
			require.Equal(t, "trapeze://skills/trapeze-hooks/SKILL.md", s.SkillFilePath)
			require.Equal(t, "trapeze://skills/trapeze-hooks", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	}
	require.True(t, foundHooks, "trapeze-hooks builtin skill not found")
}
...
func TestDeduplicate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:    []*Skill{{Name: "trapeze-config", Path: "trapeze://skills/trapeze-config"}, {Name: "trapeze-config", Path: "/user/trapeze-config"}},
			wantLen:  1,
			wantName: "trapeze-config",
			wantPath: "/user/trapeze-config",
		},
		{
			name:    "empty",
			input:   nil,
