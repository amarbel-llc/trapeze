...
// event into a closure nobody reads, and subscribers (`trapeze run`)
...
// publish a RunComplete for its RunID, hanging a `trapeze run` caller that
...
// than internal bookkeeping, which is the contract a `trapeze run` caller
...
// bookkeeping slice, not the published event a `trapeze run` caller awaits.
