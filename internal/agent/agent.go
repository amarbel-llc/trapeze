// Package agent is the core orchestration layer for Trapeze AI agents.
//
...
type SessionAgentCall struct {
	...
	// reliable completion contract (e.g. `trapeze run` against a
	...
	// event, so non-interactive clients (e.g. `trapeze run`) don't
...
// RunID, e.g. `trapeze run`, would otherwise hang). Uncanceled calls without
...
// that RunID: `trapeze run` ignores live message events and exits only on a
...
// so callers waiting on those RunIDs (e.g. `trapeze run`) are not left
...
// caller waiting on RunComplete (e.g. `trapeze run` with a RunID) always
...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (result *fantasy.AgentResult, retErr error) {
	...
	if call.Accepted != nil {
		...
		if a.canceledBySeq(call.SessionID, call.Accepted.seq) {
			...
			// RunComplete for this RunID (e.g. `trapeze run`, which
	...
	if err != nil {
		...
		if isCancelErr {
			...
			currentAssistant.AddFinish(message.FinishReasonError, "Unauthorized", `Please re-authenticate with Hyper. You can also run "trapeze auth" to re-authenticate.`)
...
func (a *sessionAgent) getCacheControlOptions() fantasy.ProviderOptions {
	if t, _ := strconv.ParseBool(os.Getenv("TRAPEZE_DISABLE_ANTHROPIC_CACHE")); t {
