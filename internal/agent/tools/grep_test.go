...
func TestGrepWithIgnoreFiles(t *testing.T) {
	...
	trapezeignoreContent := "node_modules/\n"
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".trapezeignore"), []byte(trapezeignoreContent), 0o644))
	...
	for name, fn := range map[string]func(pattern, path, include string) ([]grepMatch, error){
		...
		t.Run(name, func(t *testing.T) {
			...
			require.False(t, foundFiles["lib.js"], "Should not find lib.js (ignored by .trapezeignore)")
...
func TestSearchImplementations(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".trapezeignore"), []byte("file5.txt\n"), 0o644))
