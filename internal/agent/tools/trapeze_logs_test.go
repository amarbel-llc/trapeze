...
func createTestLogFile(t *testing.T, entries []map[string]any) string {
	t.Helper()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "trapeze.log")
...
func TestNewTrapezeLogsTool(t *testing.T) {
	t.Parallel()
	tool := NewTrapezeLogsTool("/tmp/test.log")
	require.NotNil(t, tool)
	require.Equal(t, TrapezeLogsToolName, tool.Info().Name)
}

func TestTrapezeLogs_HappyPath(t *testing.T) {
	t.Parallel()
	...
	result := runTrapezeLogs(logFile, TrapezeLogsParams{Lines: 3})
...
func TestTrapezeLogs_DefaultLines(t *testing.T) {
	t.Parallel()
	...
	result := runTrapezeLogs(logFile, TrapezeLogsParams{Lines: 0})
...
func TestTrapezeLogs_MaxCap(t *testing.T) {
	t.Parallel()
	...
	result := runTrapezeLogs(logFile, TrapezeLogsParams{Lines: 200})

	lines := strings.Split(result, "\n")
	require.Len(t, lines, 100)
}

func TestTrapezeLogs_MissingFile(t *testing.T) {
	t.Parallel()
	result := runTrapezeLogs("/nonexistent/path/trapeze.log", TrapezeLogsParams{Lines: 50})
	require.Contains(t, result, "No log file found")
}

func TestTrapezeLogs_EmptyFile(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "trapeze.log")
	...
	result := runTrapezeLogs(logFile, TrapezeLogsParams{Lines: 50})
	require.Contains(t, result, "Log file is empty")
}

func TestTrapezeLogs_MalformedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "trapeze.log")
	...
	result := runTrapezeLogs(logFile, TrapezeLogsParams{Lines: 10})
...
func TestTrapezeLogs_ExtraFieldsSorted(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runTrapezeLogs(logFile, TrapezeLogsParams{Lines: 1})
...
func TestTrapezeLogs_NonStringValues(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestTrapezeLogs_Redaction(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestTrapezeLogs_ReservedFields(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
		{
			"time":   time.Now().Format(time.RFC3339),
...
func TestTrapezeLogs_OversizedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "trapeze.log")
...
func TestTrapezeLogs_PartialTrailingLine(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "trapeze.log")
...
func TestTrapezeLogs_ValueQuoting(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestTrapezeLogs_ChronologicalOrder(t *testing.T) {
	t.Parallel()
...
func TestTrapezeLogs_TimeOnlyFormat(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestTrapezeLogs_LevelVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runTrapezeLogs(logFile, TrapezeLogsParams{Lines: 5})
...
func TestTrapezeLogs_SourceVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
