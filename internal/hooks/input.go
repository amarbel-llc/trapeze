...
func BuildEnv(eventName, toolName, sessionID, cwd, projectDir, toolInputJSON string) []string {
	env := os.Environ()
	env = append(env, shell.TrapezeEnvMarkers()...)
	env = append(
		env,
		fmt.Sprintf("TRAPEZE_EVENT=%s", eventName),
		fmt.Sprintf("TRAPEZE_TOOL_NAME=%s", toolName),
		fmt.Sprintf("TRAPEZE_SESSION_ID=%s", sessionID),
		fmt.Sprintf("TRAPEZE_CWD=%s", cwd),
		fmt.Sprintf("TRAPEZE_PROJECT_DIR=%s", projectDir),
	)

	// Extract tool-specific env vars from the JSON input.
	if toolInputJSON != "" {
		if cmd := gjson.Get(toolInputJSON, "command"); cmd.Exists() {
			env = append(env, fmt.Sprintf("TRAPEZE_TOOL_INPUT_COMMAND=%s", cmd.String()))
		}
		if fp := gjson.Get(toolInputJSON, "file_path"); fp.Exists() {
			env = append(env, fmt.Sprintf("TRAPEZE_TOOL_INPUT_FILE_PATH=%s", fp.String()))
		}
	}
...
// Supports both Trapeze format and Claude Code format (hookSpecificOutput).
