...
func TestBuildEnv(t *testing.T) {
	...
	require.Equal(t, EventPreToolUse, envMap["TRAPEZE_EVENT"])
	require.Equal(t, "bash", envMap["TRAPEZE_TOOL_NAME"])
	require.Equal(t, "sess-1", envMap["TRAPEZE_SESSION_ID"])
	require.Equal(t, "/work", envMap["TRAPEZE_CWD"])
	require.Equal(t, "/project", envMap["TRAPEZE_PROJECT_DIR"])
	require.Equal(t, "ls", envMap["TRAPEZE_TOOL_INPUT_COMMAND"])
	require.Equal(t, "/tmp/f.txt", envMap["TRAPEZE_TOOL_INPUT_FILE_PATH"])

	// Shared Trapeze markers must be present so hook-authored scripts can
	// detect they're running under Trapeze the same way bash-tool-invoked
	// scripts can.
	require.Equal(t, "1", envMap["TRAPEZE"])
	require.Equal(t, "trapeze", envMap["AGENT"])
	require.Equal(t, "trapeze", envMap["AI_AGENT"])
}
...
func TestRunnerEnvVarsPropagated(t *testing.T) {
	t.Parallel()
	hookCfg := config.HookConfig{
		Command: `printf '{"decision":"allow","context":"%s"}' "$TRAPEZE_TOOL_NAME"`,
	}
	r := NewRunner([]config.HookConfig{hookCfg}, t.TempDir(), t.TempDir())
	result, err := r.Run(context.Background(), EventPreToolUse, "sess", "bash", `{}`)
	require.NoError(t, err)
...
func TestParseStdoutClaudeCodeFormat(t *testing.T) {
	...
	t.Run("trapeze format still works", func(t *testing.T) {
		t.Parallel()
