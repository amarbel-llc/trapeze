...
// with reason=tool_use and used to terminate `trapeze run` early via
...
// subscribers (e.g. a TUI watching workspace events while `trapeze
...
// non-nil error from `trapeze run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `trapeze run` waits) exits cleanly rather
...
// queue scenario: `trapeze run --continue` attaches to a session
