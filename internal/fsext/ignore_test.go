...
func TestTrapezeIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .trapezeignore file that ignores .log files
	require.NoError(t, os.WriteFile(".trapezeignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .trapezeignore file
	trapezeignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".trapezeignore"), []byte(trapezeignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .trapezeignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .trapezeignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .trapezeignore in subdir that ignores normal_nested
	subTrapezeignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".trapezeignore"), []byte(subTrapezeignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .trapezeignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .trapezeignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .trapezeignore")
