...
func TestProviders_Integration_WithMockClients(t *testing.T) {
	...
	catwalkPath := tmpDir + "/trapeze/providers.json"
	hyperPath := tmpDir + "/trapeze/hyper.json"
...
func TestProviders_Integration_WithCachedData(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(tmpDir+"/trapeze", 0o755))
...
func TestCachePathFor(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected:    "/custom/data/trapeze/providers.json",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expected != "" {
				...
				require.Contains(t, result, "trapeze")
