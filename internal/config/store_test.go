...
func TestConfigStore_ConfigPath_GlobalAlwaysWorks(t *testing.T) {
	...
	store := &ConfigStore{
		globalDataPath: "/some/global/trapeze.json",
	}

	path, err := store.configPath(ScopeGlobal)
	require.NoError(t, err)
	require.Equal(t, "/some/global/trapeze.json", path)
}
...
func TestConfigStore_ConfigPath_WorkspaceReturnsPath(t *testing.T) {
	...
	store := &ConfigStore{
		workspacePath: "/some/workspace/.trapeze/trapeze.json",
	}

	path, err := store.configPath(ScopeWorkspace)
	require.NoError(t, err)
	require.Equal(t, "/some/workspace/.trapeze/trapeze.json", path)
}
...
func TestGlobalWorkspaceDir(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("TRAPEZE_GLOBAL_DATA", dir)
...
func TestReloadFromDisk_UsesNewConfigValues(t *testing.T) {
	...
	t.Setenv("TRAPEZE_GLOBAL_CONFIG", dir)
	t.Setenv("TRAPEZE_GLOBAL_DATA", dir)
	resetProviderState()
