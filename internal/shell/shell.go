...
// TrapezeEnvMarkers returns a fresh slice of the environment variables that
// Trapeze unconditionally sets on every shell it spawns — both the interactive
...
func NewShell(opts *Options) *Shell {
	...
	// Allow tools to detect execution by Trapeze.
	env = append(env, TrapezeEnvMarkers()...)
