...
type RunOptions struct {
	...
	// the Trapeze process cwd — hooks and the bash tool have different
...
// Trapeze handler stack. Shared by the stateless [Run] entrypoint and the
...
// regardless of the caller's environment because Trapeze shells are never
...
//  1. builtins first (so Trapeze's in-process jq wins over any PATH binary);
...
// builtinHandler returns middleware that dispatches recognized Trapeze
