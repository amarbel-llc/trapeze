...
// already in use by another trapeze process.
var ErrDataDirLocked = errors.New("data directory already in use by another trapeze process")

// dataDirLockFile is the name of the lock file inside the data
// directory. It lives next to trapeze.db so users can `ls` and find it.
const dataDirLockFile = "trapeze.lock"
...
// {dataDir}/trapeze.lock. If the lock is already held by another
...
// TRAPEZE_SKIP_DATADIR_LOCK is set to a truthy value. This is intended
...
func skipDataDirLock() bool {
	v, _ := strconv.ParseBool(os.Getenv("TRAPEZE_SKIP_DATADIR_LOCK"))
	return v
}
