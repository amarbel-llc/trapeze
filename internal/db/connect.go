...
// trapeze process from racing the storage.
...
// workspace-bootstrap path opts in. TRAPEZE_SKIP_DATADIR_LOCK still
...
func Connect(ctx context.Context, dataDir string, opts ...ConnectOption) (*sql.DB, error) {
	...
	dbPath := filepath.Join(dataDir, "trapeze.db")
	...
	// trapeze process on the same SQLite file. The lock is released when
	...
	// local-mode invocations do not refuse a second trapeze against the
